...
const DARAJA_ENV      = defineSecret('DARAJA_ENV');
...
function formatPhone(raw: string): string {
  ...
  if (d.startsWith('+254')) return d.slice(1);
...
async function getAccessToken(consumerKey: string, consumerSecret: string, base: string): Promise<string> {
...
const cache = new Map<string, { checkoutRequestId: string; expiresAt: number }>();

// ── stkPush (callable) ────────────────────────────────────────────────────────
// Called from the frontend via httpsCallable(functions, 'stkPush').
// Returns { checkoutRequestId, customerMessage }.
export const stkPush = onCall(
  { secrets: [CONSUMER_KEY, CONSUMER_SECRET, SHORTCODE, PASSKEY, DARAJA_ENV] },
  async (request) => {
    ...
    if (!phone || !amount || !orderId) {
      throw new HttpsError('invalid-argument', 'phone, amount, and orderId are required');
    }
    ...
    const data = (await mpesaRes.json()) as {
      ResponseCode?: string;
    ...
    if (!mpesaRes.ok || data.ResponseCode !== '0') {
      throw new HttpsError('internal', data.errorMessage ?? 'STK push failed');
    }
...
export const mpesaStatus = onCall(
  { secrets: [CONSUMER_KEY, CONSUMER_SECRET, SHORTCODE, PASSKEY, DARAJA_ENV] },
  async (request) => {
    const { checkoutRequestId } = request.data as { checkoutRequestId: string };
...
export const mpesaCallback = onRequest(async (req, res) => {
  try {
    ...
    if (stkCallback) {
      const { CheckoutRequestID, ResultCode, ResultDesc, CallbackMetadata } = stkCallback as {
        CheckoutRequestID: string;
        ResultCode: number;
        ResultDesc: string;
